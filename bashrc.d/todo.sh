#!/usr/bin/env bash
#
# todo.sh — todo.txt command-line interface (sourced into the interactive shell).
#
# Defines the `todo` function supporting add, list, do (complete), delete, open,
# and regex filtering by priorities/projects/contexts. Falls back to ~/todo.txt
# when no local todo file is present.

_todo_find_file() {
	local pwd_todo="todo.txt"
	local -a pwd_custom_todos=()

	while IFS= read -r line; do
		pwd_custom_todos+=("$line")
	done < <(find . -maxdepth 1 -name "*.todo.txt" 2>/dev/null | sort)
	...
	if [[ -f "$pwd_todo" ]]; then
		if [[ ${#pwd_custom_todos[@]} -gt 0 ]]; then
			...
			read -r -p "Select file (0-${#pwd_custom_todos[@]}): " choice >&2
			if [[ "$choice" -eq 0 ]]; then
				echo "$pwd_todo"
			else
		...
		read -r -p "Select file (1-${#pwd_custom_todos[@]}): " choice >&2
		echo "${pwd_custom_todos[$((choice - 1))]}"
	else
		echo "$HOME/todo.txt"
	fi
